//nvcc -arch=sm_35 -rdc=true 17.cu -o 17

// Цикл в ядре выполняется до 50 итераций и предназначен для
// сопоставления векторов атрибутов X векторам классов y

// Атрибутами X - длина чашелистника, 
// 				ширина чашелистника, 
// 				длина лепестка, 
// 				ширина лепестка. 
// В примере будет использоваться 2 из 3х видов растений ирисов 
// – это Ирис щетинистый (Iris setosa) и Ирис виргинский (Iris virginica). 
// За прогнозирование отвечает вектор pred.
// Размер матрицы X есть размер партии по количеству атрибутов.


#include <stdio.h>

__global__ void kMartixByMatrixElementwise(
	const int nThreads, 
	const float *m1, 
	const float *m2, 
	float *output) 
...
m1: array
m2: array
output: array,the results 
		of the multiplication are to be stored here
*/
...
__device__ float* dMartixByMatrixElementwise(
		const float *m1, 
		const float *m2, 
		float *output, 
		const int width, 
...
__global__ void kMartixSubstractMatrix(
	const int nThreads, 
	const float *m1,
	const float *m2, 
	float *output) 
...
m1: array
m2: array
output: array,the results of the 
		computation are to be stored here
*/
	for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < nThreads; i += blockDim.x * gridDim.x)
	{
		output[i] = m1[i] - m2[i];
...
__device__ float* dMartixSubstractMatrix(
	const float *m1, 
	const float *m2,
	float *output, 
	const int width, 
...
__global__ void kSigmoid_d(
	const int nThreads, 
	float const *input, 
	float *output) 
{
/* Computes the value of the sigmoid function derivative 
f'(x) = f(x)(1 – f(x)),
where f(x) is sigmoid function.
Inputs:
input: array
output: array, the results of the computation are to be stored
here:
...
// перемножения двух матриц X и W0
__global__ void kDot(
	const int nThreads, 
	const float *m1, 
	const float *m2,
	float *output, 
...
m2: array, right matrix of size m1_columns x m2_columns (the num-
ber of rows in the right matrix
must be equal to the number of the columns in the left one)
output: array, the results of the computation are to be stored
here:
m1 * m2, product of two arrays m1 and m2, a matrix of
size m1_rows x m2_columns
...
// умножает матрицу на транспонированную матрицу
__global__ void kDot_m1_m2T(
	const int nThreads, 
	const float *m1, 
	const float *m2, 
	float *output, 
	...
	const int m2_rows)
{
/* Updates the output matrix with the product of two matrices: 
m1 and m2 transposed.
Inputs:
m1: array, left matrix of size m1_rows x m1_columns
m2: array, right matrix of size m2_rows x m1_columns (m2 trans-
posed will be of size m1_columns x m2_rows)
output: array, the results of the computation are to be stored
here:
m1 * m2, product of two arrays m1 and m2, a matrix of
size m1_rows x m2_rows
m1_columns: int, number of columns in the left matrix m1
...
__global__ void kDot_m1T_m2(
	...
	const int m2_columns )
{
/* Increments the output matrix with the product of two matrices: m1
transposed and m2.
Inputs:
m1: array, left matrix of size m1_rows x m1_columns (m1 trans-
posed will be of size m1_columns x m1_rows)
m2: array, right matrix of size m1_rows x m2_columns
output: array, the results of the computation are to be stored
here:
m1 * m2, product of two arrays m1 and m2, a matrix of
size m1_columns x m2_columns
m1_rows: int, number of rows in the left matrix m1
m1_columns: int, number of columns in the left matrix m1
m2_rows: int, number of rows in the left matrix m2
*/
	...
	float *buffer)
{
	...
	{
		/*
		Для того, чтобы найти значения скрытого слоя
		Для вычисления скрытого слоя, в первую очередь, нам нужно умножить
		4x4 матрицы X на 4x4 матрицы W0. Тогда нам нужно будет применить
		функцию активации; в этом случае мы будем использовать сигмоидную функцию.<
		*/
		dSigmoid(dDot(X, W0, l1, X_h, X_w, l1_w), l1, X_h, l1_w);
		/*
		перемножить матрицы 4х8 l1 и 8х1 W1 и применить функцию активации:
		*/
		dSigmoid(dDot(l1, W1, pred, X_h, l1_w, y_w), pred, X_h, y_w);
		/*
		Результатом перемножения станет матрица с прогнозированием.
		Определить вектор ошибок прогнозирования pred_d. 
		
		Ядро CUDA, которое находит разницу между двумя матрицами:
		__global__ void kMartixByMatrixElementwise

		Элементный мультипликатор для двух векторов:
		__global__ void kMartixByMatrixElementwise

		*/
		dMartixByMatrixElementwise(
			dMartixSubstractMatrix(y, pred, pred_d,X_h, y_w), 
			dSigmoid_d(pred, buffer, X_h, y_w), 
			pred_d, X_h, y_w );
		/*
		Сначала умножим pred_d на транспонированную W1. 
		Затем вычислим сигмоид (l1) и, наконец, 
		умножим результат этих двух операций:
		*/
		dMartixByMatrixElementwise(
			dDot_m1_m2T(pred_d, W1, l_1_d, X_h, y_w, l1_w), 
			dSigmoid_d(l1, buffer, X_h, l1_w), 
			l_1_d, X_h, l1_w);
		/*
		Обновить веса W0 с результатом матричного умножения 
		транспонированных X и l_1_d
		*/
