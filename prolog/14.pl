%% Три миссионера и три людоеда должны пересечь реку на лодке, 
%% способной выдержать не более двух человек. При этом на одном 
%% берегу не может оставаться больше людоедов, чем миссионеров 
%% (иначе миссионеров съедят). Лодка также не может пересечь реку без людей на борту.
:- use_module(library(clpfd)).

legal_coast(coast(MisCount, CanCount)) :-
    MisCount >= 0, MisCount =< 3,
    CanCount >= 0, CanCount =< 3,
...
on_right(move(MisMoved, CanMoved), coast(MisLeft, CanLeft), coast(MisNewLeft, CanNewLeft)) :-
    MisNewLeft is MisLeft - MisMoved,
    CanNewLeft is CanLeft - CanMoved.

on_left(move(MisMoved, CanMoved), coast(MisLeft, CanLeft), coast(MisNewLeft, CanNewLeft)) :-
    MisNewLeft is MisLeft + MisMoved,
    CanNewLeft is CanLeft + CanMoved.
...
update_people(Move, left, Left, NewLeft) :- on_right(Move, Left, NewLeft).
update_people(Move, right, Left, NewLeft) :- on_left(Move, Left, NewLeft).
